  ...
  <v-dialog class="student_resource" max-width="800px">
    ...
    <template v-slot:default="dialog">
        ...
        <div class="student_toolbar">
          <v-toolbar color="white" elevation="0">
            ...
            <v-toolbar-title
              ...
              "
            v-text="interactiveMenu[report['name']]['title']"
            >
            </v-toolbar-title>

            <v-spacer></v-spacer>
            <div ckass="pl-20">
              <v-icon class="icon_close dark" @click="dialog.value = false"
                >mdi-close</v-icon
              >
            </div>
          </v-toolbar>
     ...
     <v-card-text>
          <v-card
            elevation="0"
            v-for="(data, i) in interactiveMenu[report['name']][
              report['level']]"
            :key="i"
          >
            <v-card-title
              align="left"
              class="
                title-report-sm title-report-md title-report-lg title-report-xl
                text-truncate
              "
              >{{ data.subtitle }}</v-card-title
            >
            <img
              :src="require('@/assets/img/icon_menu.png')"
              height="15"
              class="spacer-center icon-subtitle"
            />
            <v-card-text
              v-for="(c, i) in data.content"
              :key="i"
              class="
                py-1
                text-body1-sm text-body1-md text-body1-lg text-body1-xl
              "
            >
              <div>· {{ c }}</div>
            </v-card-text>
