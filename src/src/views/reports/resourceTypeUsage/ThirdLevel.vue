    ...
    <template v-else>
      ...
      <v-row>
        ...
        <v-col cols="12" sm="2" class="d-flex justify-center justify-sm-end ">
          <help :report="{name:'resource_type_usage', level:'first' ,log_level:'third_level'}" />
          <download :report="{name:'resource_type_usage', level:'third',log_level:'third_level'}" :data="students" :title="title" />
        </v-col>
      </v-row>

      <template v-if="no_problem">
          <v-data-table :headers="headers" :items="students" item-key="member_canvas_id" hide-default-footer
            :items-per-page="-1" :sort-by.sync="sort_by" :sort-desc.sync="sort_desc" :search="search">
...
export default {
  ...
  methods: {
    ...
    currentVariables() {
      this.current_name_resource = this.$route.params.type;
      this.current_resource_id = parseInt(this.$route.params.resource);
    },
    startReport() {
    ...
    filterResources() {
      this.name_resource = this.$route.params.type;
      let resources_item =
        this.information[this.name_resource]["resources_interactions"];
      let resources = this.information[this.name_resource]["resources"];
    ...
    filterReportInformation(students) {
      ...
      if (Object.entries(students).length != 0) {
        ...
        this.warning["message"] = " El curso no posee estudiantes";
