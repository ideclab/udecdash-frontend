  ...
  <div class="report_container">
    ...
    <template v-else>
      <template v-if="no_problem">
        <router-view :information="resource_visualization"  class="py-5 px-1 px-sm-5"></router-view>
      </template>
      <template v-else>
...
export default {
  ...
  methods: {
    startReport() {
      ...
      webservice
        .resourceVisualizations(current_course_id)
        .then(response => {
          if (!response.error) {
           this.resource_visualization = response.information
          } else {
